...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for index, x in enumerate(feature_matrix):
        z = labels[index] * (np.inner(theta, x) + theta_0)
        if z < 1:
            suma = suma + 1 - z
...
def perceptron_single_step_update(
    ...
    if label*(np.inner(current_theta,feature_vector)+current_theta_0) <= np.finfo(float).eps:
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            # Your code here
            theta, theta_0 = pegasos_single_step_update(feature_matrix[i], labels[i], L, 1 / np.sqrt(count), theta,
                                                        theta_0)
            count += 1
            pass
    return theta, theta_0
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(feature_matrix.shape[0]):
        if np.inner(theta, feature_matrix[i]) + theta_0 > np.finfo(float).eps:
            labels[i] = 1
        else:
            labels[i] = -1
    return labels
